...
class Test(unittest.TestCase):
    ...
    def test_targeted_audience(self):
        ...
        self.assertEqual("3回生以上", targeted_audience)

    def test_subject_outline(self):
        subject_outline = Test.page.get_subject_outline()

        text = """生体および細胞にとって恒常性維持は生存に不可欠である。通常とは異なる状況が生じたときに細胞は，異常を細胞内に存在するセンサーによって感知し，核へ情報を伝達して遺伝子発現を調節することによって適応する仕組みを備えている。このような細胞内情報発信では，レセプターを介した細胞表面からの情報伝達の場合とは全く異なる分子機構が用いられている。本講義では，低酸素，コレステロール濃度低下，分泌系タンパク質の構造異常（小胞体ストレス）などの異常事態に対して細胞が，どのように変化を感知し，どのように核へ向けて情報を伝達し，核内でどのような遺伝子発現を促して恒常性を維持するのか解説する。
        ...
        print(text)
        print()
        print(subject_outline)

        self.assertEqual(text, subject_outline)
